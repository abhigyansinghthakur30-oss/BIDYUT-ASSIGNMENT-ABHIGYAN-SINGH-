...
export default function AdminRegisterPage() {
  ...
  const [adminCode, setAdminCode] = useState(""); // Simple extra safeguard
    ...
    <div className="flex items-center justify-center min-h-screen p-6 bg-gradient-to-br from-[#0a0a1f] via-[#12122b] to-[#0a0a1f]">
      <div className="w-full max-w-md animate-fade-in">
        <div className="mb-10 text-center">
          ...
          <p className="text-slate-400">Initialize a new administrative account</p>
        </div>
        ...
        <div className="p-8 border glass-card border-white/10">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-300 italic">Auth Code (BIDYUT2024)</label>
              <input
                type="text"
            ...
            <button type="submit" className="w-full py-4 btn-primary" disabled={loading}>
              {loading ? "Creating..." : "Set Up Admin Account"}
            </button>
          </form>

          <p className="mt-8 text-sm text-center text-slate-500">
            Already have admin access? <Link href="/login/admin" className="font-bold text-purple-400 hover:text-purple-300 transition-colors">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
